...
class Connection:
    ...
    def insertNew(self, uid, credit=1, email='user'):
        print "insert new"
        self.c.execute(
            'INSERT OR IGNORE INTO users(tag, credit, name, creation_date) VALUES( {}, {}, "{}", datetime("now"))'.format(
                uid,credit, email))
        self.conn.commit()
        self.insertNext = False

    def removeCredits(self, uid):
